...
>
    <a
        ...
        @endif
        @class([
            'fi-sidebar-item-button relative flex items-center justify-center gap-x-3 rounded-lg px-2 py-2 outline-none transition duration-75 hover:bg-white-500 focus-visible:bg-white-500 dark:hover:bg-white/5 dark:focus-visible:bg-white/5',
            'bg-white dark:bg-white-500' => $active,
        ])
    >
        ...
        @if ((blank($icon) && $grouped) || $subGrouped)
            ...
            >
                @if (! $first)
                    <div
                        class="absolute -top-1/2 bottom-1/2 w-px bg-white-500 dark:bg-white-500"
                    ></div>
                @endif

                @if (! $last)
                    <div
                        class="absolute -bottom-1/2 top-1/2 w-px bg-white-500 dark:bg-white-500"
                    ></div>
                @endif
                ...
                <div
                    @class([
                        ...
                        'bg-white-500 dark:bg-white-500' => ! $active,
